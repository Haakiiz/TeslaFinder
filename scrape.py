...
class Listing:
    ...
    def from_card(cls, card_html: str) -> "Listing | None":
        if " km" in card_html and "∙" in card_html:
            # shows us the exact Unicode stepping
            snippet = card_html.split(" km")[0][-60:]
        ...
        try:
            ...
            mileage = 0
            ym_match = re.search(
                r'(\d{4}).{0,20}?([0-9][0-9\s;&nbsp;]+)\s*km',  # tolerate &nbsp; or NBSP
                card_html,
                re.IGNORECASE,
            )
            if ym_match:
            ...
            # --- Color (heuristic from thumbnail description; often absent in search view) ---
            color = ""
